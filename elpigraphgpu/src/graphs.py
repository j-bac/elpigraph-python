    ...
    if(Structure == 'branches'):
        ...
        if(np.any(np.array(Net.degree())>2) & np.any(np.array(Net.degree())==1)):
            ...
            for i in range(len(SelEp)-1):
                AllPaths.extend(
                    Net.get_shortest_paths(SelEp[i],to = SelEp[range(i+1,len(SelEp))], output = "vpath")
                    )

            Valid = np.array(list(map(lambda x: np.sum(np.array(Net.degree(x)) != 2), AllPaths))) == 2
            ...
            if(len(StillToCapture)>0):
                ...
                for i in range(len(StillToCapture)):
                    ...
                    PointDists = tNet.shortest_paths(StillToCapture[i],
                ...
                for i in range(NewBrEP.shape[1]):
                    ...
                    tNet = Net.induced_subgraph(set(StillToCapture[np.all(np.isin(EndPoints, NewBrEP[:,i]),axis=0)]).union(NewBrEP[:,i]))
                    ...
                    if(len(PotentialEnds)==1):
                        ...
                        # get all loops
                        AllLoops = tNet.get_isomorphisms_vf2(igraph.Graph.Ring(tNet.vcount(), directed = False, circular = True))
                    ...
                    if(len(PotentialEnds) == 2):
                        ...
                        LinePath = tNet.get_shortest_paths(PotentialEnds[1], PotentialEnds[2])
                        ...
                        if(len(LinePath) < tNet.vcount()):
                            ...
                            AllLoops = tNet.get_subisomorphisms_vf2(tNet, igraph.Graph.Ring(tNet.vcount()-len(LinePath)+1, directed = False, circular = True))

                            if(len(AllLoops) == 0):
                                raise ValueError("Unsupported structure. Contact the package maintainer")
        ...
        if(np.any(np.array(Net.degree())>2) & np.any(np.array(Net.degree())==1)):
            ...
            for i in BrPoints:
                ...
                for j in SelEp:
                    ...
                    if(not any(np.isin(np.array(Path), np.array(list(set(BrPoints).difference([i,j])))))):
                        Allbr.append(Path)
    ...
    if(Structure == 'end2end'):
        ...
        for i in range(len(EndPoints)-1):
            for j in range(i+1,len(EndPoints)):
                Path = Net.get_shortest_paths(EndPoints[i], to = EndPoints[j],output='vpath')
                if(len(Path) > 0):
                    Allbr.append(Path)
...
def GetBranches(Net, StartingPoints = None):
    ...
    while (len(tNet.vs.indices)>0):
        ...
        for i in range(len(CurrentEdges)):
            AllNei = [tNet.vs['name'][k] for k in tNet.neighborhood(vertices=tNet.vs['name'].index(CurrentEdges[i]), order = 1)]
