...
def CollapseBranches(X,
    ...
    # get the partition
    PartStruct = PartitionData(X = X, NodePositions = TargetPG['NodePositions'],MaxBlockSize=100000000, TrimmingRadius = TrimmingRadius,SquaredX=np.sum(X**2,axis=1,keepdims=1))
...
def ShiftBranching(X,
    ...
    PD = PartitionData(X = X, NodePositions = TargetPG['NodePositions'],MaxBlockSize= 100000000,TrimmingRadius = TrimmingRadius,
    ...
    for br in BrIds:
        ...
        if(NewBR != br):
            ...
            with warnings.catch_warnings():
                ...
                AllPath_vpath = Net.get_shortest_paths(NewBR, to = ToReconnect, output='vpath')
            ...
            ToReconnect = list(set(ToReconnect).difference([j for i in AllPath_vpath for j in i]))
