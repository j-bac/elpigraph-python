...
def PartitionData_cp(Xcp, NodePositions, MaxBlockSize,SquaredXcp,
...
def ComputeSpringLaplacianMatrix(ElasticMatrix):
    ...
    for i in range(StarCenterIndices.size):
        ...
        tmp = np.repeat(leaves, ElasticMatrix.shape[0]).reshape((-1,ElasticMatrix.shape[0]))
...
def ComputeRelativeChangeOfNodePositions(NodePositions, NewNodePositions):
    ...
    #' @examples
    '''
    return (((NodePositions - NewNodePositions)**2).sum(axis=1) / (NewNodePositions**2).sum(axis=1)).max()
...
def PrimitiveElasticGraphEmbedment_cp(X, NodePositions, ElasticMatrix,
                                   MaxNumberOfIterations=10, eps=0.01,
                                   Mode = 1, FinalEnergy = "Base",
                                   alpha = 0,
                                   beta = 0,
